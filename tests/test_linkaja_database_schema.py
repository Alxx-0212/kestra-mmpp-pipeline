...
class LinkAjaDatabaseSchemaContractTest(unittest.TestCase):
    ...
    def test_monthly_workflow_publishes_database_analysis_files_only(self):
        workflow_path = (
            Path(__file__).parents[1] / "linkaja_monthly_materialization.yml"
        )
        normalized = " ".join(
    ...
    def test_daily_workflow_persists_results_without_sheet_side_effects(self):
        workflow_path = Path(__file__).parents[1] / "linkaja_fee_pipeline.yml"
        normalized = " ".join(
            workflow_path.read_text(encoding="utf-8").lower().split()
        )
    ...
    def test_linkaja_runtime_api_and_dependencies_are_sheet_free(self):
        self.assertFalse(hasattr(linkaja_pipeline, "make_gspread_client"))
        self.assertFalse(
            hasattr(linkaja_pipeline, "process_linkaja_fee_sheet_upload")
        )
        self.assertFalse(
            hasattr(linkaja_pipeline, "process_linkaja_detail_sheet_upload")
        )

        requirements_path = (
            Path(__file__).parents[1] / "requirements-linkaja.txt"
        )
