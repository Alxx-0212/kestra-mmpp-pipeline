/*
   FinPay transaction evidence and guarded reversal model.

   The existing daily tables remain compatibility/reporting tables. These
   relations retain source generations and expose transaction-grain views for
   the monthly settlement consumer.
*/
...
WITH base_events AS (
    SELECT *
    FROM finpay_transaction_events_current_v
), reversal_matches AS (
    SELECT
        reversal.event_id,
    ...
    SELECT
        matches.*,
        CASE
            WHEN original_candidate_count = 0
                THEN 'UNRESOLVED_MISSING_ORIGINAL'
            WHEN prior_original_count = 0
                THEN 'INVALID_CHRONOLOGY'
            WHEN prior_original_count > 1
                THEN 'AMBIGUOUS_MULTIPLE_ORIGINALS'
    ...
    JOIN base_events AS reversal
      ON reversal.event_id = matches.event_id
    ...
    LEFT JOIN reversal_status AS status
      ON status.event_id = reversal.event_id
    ...
    SELECT
        event.*,
        CASE
            WHEN event.event_role = 'REVERSAL'
                THEN status.original_candidate_count
            ELSE NULL
        END AS original_candidate_count,
        CASE
            WHEN event.event_role = 'REVERSAL'
                THEN status.prior_original_count
            ELSE NULL
        END AS prior_original_count,
        CASE
            WHEN event.event_role = 'REVERSAL'
                THEN status.prior_signed_amount
            ELSE NULL
        END AS prior_signed_amount,
        COALESCE(original_info.valid_reversal_count, 0)::INTEGER
            AS valid_reversal_count,
        COALESCE(original_info.all_reversal_count, 0)::INTEGER
            AS all_reversal_count,
        status.reversal_resolution_status AS reversal_status
    FROM base_events AS event
    LEFT JOIN reversal_status AS status
      ON status.event_id = event.event_id
    LEFT JOIN original_reversals AS original_info
      ON original_info.event_id = event.event_id
)
SELECT
    enriched.*,
    CASE
        WHEN event_role = 'ORIGINAL' AND valid_reversal_count > 1
            THEN 'MULTIPLE_REVERSALS'
        WHEN event_role = 'ORIGINAL'
            THEN 'NOT_REVERSAL'
        ELSE reversal_status
    END AS reversal_resolution_status,
    (
        event_role = 'ORIGINAL'
        AND valid_reversal_count > 0
    ) AS is_reversed,
    (
        event_role = 'ORIGINAL'
        AND valid_reversal_count = 0
    ) AS active_without_cutoff,
    (
        CASE
